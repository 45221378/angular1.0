...
(function ($, win, undef) {
    ...
    var tipmsg = {//默认提示文字;
        ...
        w: {
            ...
            "url": "网址输入有误！",
            "name": "姓名输入有误！",
            "namecompare":"与其他联系人姓名重复",
            "phonecompare":"与其他联系人手机号码重复",
            "idcard": "身份证号输入有误！",
            "bank": "银行卡号输入有误！",
            "bankName": "开户银行输入有误！",
            "msgcode":"短信验证码输入有误！",
            "imgcode":"图形验证码输入有误！",
            "messageCode":"短信验证码输入有误！",
            "phonePassword":"手机服务密码输入有误！"
        },
        nullmsg: {
            "name": "请输入姓名！",
            "idcard": "请输入身份证号！",
            ...
            "m": "请输入手机号码！",
            "msgcode":"请输入短信验证码！",
            "messageCode":"请输入短信验证码！",
            "phonePassword":"请输入手机服务密码"
        },
        def: "请填写正确信息！",
        undef: "datatype未定义！",
        reck: "两次输入的内容不一致！",
    ...
    Validform.util = {
        dataType: {
            ...
            "msgcode": /^(\d{6})$/,
            "imgcode": /[\w\W]{4,20}/,
            "messageCode": /[\w\W]{4,20}/,
            "phonePassword": /^(\d{4,20})$/,
            "bank":/^[0-9]{16,19}$/,
            "bankName":/^([\u4E00-\u9FA5]{2,})$/,
            "name":/^([\u4E00-\u9FA5]{2,10})$/,
            "namecompare":function(gets,obj,curform,regxp){
                var compareArr=$('.'+obj.data('compare'));
                var flag=true,msg='姓名重复',label=obj.prev().html();
                if(!Validform.util.dataType.name.test(gets)){
                    return label+'输入有误';
                }
                compareArr.each(function(key, index){
                   var $this=$(this),value=$this.val(),text=$this.prev().html();
                    if ((text!=label)&&value==gets){
                        flag=false;
                        msg=label+'与'+text+'不能相同';
                        return false;
                    }
                });
                return flag?true:msg;
            },
            "phonecompare":function(gets,obj,curform,regxp){
                var compareArr=$('.'+obj.data('compare'));
                var flag=true,msg='手机号码重复',label=obj.prev().html();
                if(!Validform.util.dataType.m.test(gets)){
                    return label+'输入有误';
                }
                compareArr.each(function(key, index){
                    var $this=$(this),value=$this.val(),text=$this.prev().html();
